...
const db = mysql.createConnection({
  ...
  database: "samg_db", // Asegúrate de que esta base de datos exista
});
...
app.post('/cambiar-contrasena', (req, res) => {
    const { id_usuario, oldPassword, newPassword } = req.body;

    if (!id_usuario || !oldPassword || !newPassword) {
        return res.status(400).json({ message: 'Todos los campos son obligatorios.' });
    }

    // Verificar si la contraseña actual es correcta
    const query = 'SELECT * FROM usuario WHERE id_usuario = ? AND password = ?';
    
    db.query(query, [id_usuario, oldPassword], (err, results) => {
        if (err) {
        ...
        const updateQuery = 'UPDATE usuario SET password = ? WHERE id_usuario = ?';
        
        db.query(updateQuery, [newPassword, id_usuario], (err2, result2) => {
            if (err2) {
